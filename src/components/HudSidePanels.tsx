...
const HudSidePanels = () => {
  ...
  return (
    <>
      {/* LEFT PANEL */}
      <div className="fixed left-3 top-1/2 -translate-y-1/2 z-10 pointer-events-none flex flex-col gap-3 w-44 animate-fade-in-up">
        {/* Circular radar */}
        ...
        <div className="border border-border/20 bg-card/20 backdrop-blur-sm rounded-sm p-2 space-y-1.5">
          <div className="font-mono text-[8px] text-primary/60 tracking-widest">SYSTEM STATS</div>
          {[
      ...
      {/* RIGHT PANEL */}
      <div className="fixed right-3 top-1/2 -translate-y-1/2 z-10 pointer-events-none flex flex-col gap-3 w-44 animate-fade-in-up">
        {/* Time & Date */}
        <div className="border border-border/20 bg-card/20 backdrop-blur-sm rounded-sm p-2 text-right">
          <div className="font-orbitron text-lg text-primary tracking-wider">{formatTime(time)}</div>
        ...
        <div className="border border-border/20 bg-card/20 backdrop-blur-sm rounded-sm p-2">
          ...
          <div className="flex items-center justify-center gap-[2px] h-10">
            {Array.from({ length: 20 }).map((_, i) => (
              <div
                key={i}
                className="w-[3px] bg-primary/50 rounded-full animate-pulse-glow"
                style={{
                  height: `${4 + Math.sin(i * 0.5 + Date.now() * 0.001) * 12 + Math.random() * 6}px`,
        ...
        <div className="flex gap-2 justify-center">
          {[
            ...
            <div key={ring.label} className="relative w-12 h-12">
              <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
                ...
                <circle
                  ...
                  stroke={ring.color}
                  strokeWidth="2"
                  strokeDasharray={`${ring.value * 0.942} 94.2`}
                  strokeLinecap="round"
                  opacity="0.7"
                />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="font-mono text-[7px] text-primary/70">{ring.label}</span>
                <span className="font-orbitron text-[8px] text-primary">{ring.value}</span>
              </div>
            </div>
          ))}
        </div>
