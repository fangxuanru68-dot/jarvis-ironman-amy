...
const FaceHandTracker = ({ videoElement, isActive, onGesture }: FaceHandTrackerProps) => {
  ...
  useEffect(() => {
    ...
    const detect = () => {
      ...
      if (now - lastTimestamp < 100) { // ~10fps for detection
        animFrameRef.current = requestAnimationFrame(detect);
        return;
      }
      lastTimestamp = now;
      ...
      try {
        ...
        if (faceDetectorRef.current) {
          ...
          if (faceResult.detections?.length > 0) {
            ...
            if (bb) {
              ...
              setFaceBox({
                x: 1 - (bb.originX + bb.width) / videoElement.videoWidth,
                y: bb.originY / videoElement.videoHeight,
                w: bb.width / videoElement.videoWidth,
                h: bb.height / videoElement.videoHeight,
              });
            }
          } else {
            setFaceBox(null);
        ...
        if (handDetectorRef.current) {
          ...
          if (handResult.landmarks?.length > 0) {
            ...
            const mirrored = handResult.landmarks[0].map((lm: HandLandmark) => ({
              ...lm,
              x: 1 - lm.x,
            }));
  ...
  }, [isActive, videoElement, detectGesture, onGesture]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Draw face box with scanning effect
      if (faceBox) {
        ...
        const lines = [
          "ID: VERIFIED",
          "CONF: 98.7%",
          `POS: ${Math.round(faceBox.x * 100)}, ${Math.round(faceBox.y * 100)}`,
          "STATUS: ACTIVE",
        ];
        lines.forEach((line, i) => {
          ctx.globalAlpha = 0.7;
          ctx.fillText(line, dataX, by + 12 + i * 14);
        });
        ctx.globalAlpha = 1;
      }
      ...
      if (handPoints.length > 0) {
        ...
        // Finger tip tracking cursor on index finger
      ...
      if (gestureLabel) {
        ...
        ctx.fillText(text, w / 2 - textWidth / 2, h * 0.15);
        ctx.shadowBlur = 0;
      }
    };

    const interval = setInterval(draw, 33); // ~30fps render
